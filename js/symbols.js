...
const G_NO_CONT = `<path d="M0,0 V5.2 M0,20 V13 M0,13 L-4.8,5"/>` +
  `<path d="M-1.8,7 A1.8,1.8 0 0 1 1.8,7"/>`;   // 固定接点端に接し、開口は可動接点側
// 接触器主接点1極 (ブレーク) — 固定接点バー上に接触器機能の小円弧
const G_NC_CONT = G_NC + `<path d="M-5,7 A1.8,1.8 0 0 1 -1.4,7"/>`;   // 固定接点バー端に接し、開口は可動接点側
// 遮断器1極 (接点 + ×)
const G_CB = G_NO + `<path d="M-1.8,5.2 L1.8,8.8 M-1.8,8.8 L1.8,5.2"/>`;   // 自動引外しは固定接点側
// サーマル1極 (熱動素子の凸形迂回)
const G_OL = `<path d="M0,0 V7 M0,7 H-3 V13 H0 M0,13 V20"/>`;
// サーキットプロテクタ1極 (遮断器の×+熱動素子を導体に直列)
const G_CP = `<path d="M0,0 V7 M0,13 L-4.8,5 M0,13 V14 H-3 V19 H0 V20"/>` +
