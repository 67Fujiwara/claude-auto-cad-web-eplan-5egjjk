...
/* アプリの版数。ヘッダーのアプリ名の横に V1・V2 … と出す (小数点なし)。
   機能追加・修正を配布するたびに 1 つ上げること */
const APP_VERSION = 1;
...
function buildIoScaffold(page, dev) {
  ...
  /* レールは最終行の 5mm 下まで通す。最下段のコモン (や分岐) の合流点が
     レールの「端」だと角 (2 方向) 扱いで接続点の黒丸が付かず、すぐ下の
     電位リンクの三角と合わさって「黒丸の無い T 分岐」に読めてしまう
     (JIS C 0617-1 の接続点表示)。5mm 延ばすと合流点が線の途中に乗り、
     上の行と同じ黒丸が自動で付く */
  rail(comX, tags.supply, yAll + 5);                    // 外側 = コモン側
  rail(branchX, tags.branch, yIo + 5);                  // 内側 = 各行の分岐もと
...
const SPEC_SHEET2 = [
  ...
  { title: "冷却・外部接続", blocks: [
    ...
    /* 外部 I/F の施工範囲。ケーブル準備のときの端末処理は顧客に確認して
       下の記入欄 (端末処理) に書き足す */
