/* DXF に出した遮へい (破線の長円) が、画面と同じ線種として開けることを確かめる。
   ...
   ・輪郭を作る全エンティティ (LINE / ARC) が同じ LTYPE を持つこと
   ・その LTYPE のパターンが、画面の stroke-dasharray と一致すること
     (既製の DASHED = 0.25mm 線用の 3/0.75 を当てると別の線種になる)
...
const R = await p.evaluate(() => {
  ...
  out.seen = [...seen];
  // 破線を掛けた要素が、どれも破線パターンの周期以上の長さを持つか
  out.tooShort = [];
  [...arcs, ...lines].forEach(e => {
    if (!e.lt || !/^DASH/.test(e.lt)) return;
    const per = (out.ltypes[e.lt] || []).reduce((s, v) => s + Math.abs(v), 0);
    const len = e.r !== undefined
      ? e.r * (((e.a1 - e.a0 + 360) % 360) || 360) * Math.PI / 180
      : Math.hypot(e.x1 - e.x0, e.y1 - e.y0);
    if (len < per) out.tooShort.push(`${e.lt} ${len.toFixed(2)}mm < 周期 ${per.toFixed(2)}mm`);
  });
  // ARC の形が画面の外形と一致するか
  const ref = [];
  devs.forEach(dev => {
...
const checks = {
  arcAsArc: R.arcCount === 8,                    // 遮へい 4 個 × 端部 2 個
  arcDashed: R.seen.length > 0 && R.seen.every(n => /^DASH_/.test(n)),
  dashedLines: R.dashedLines === 8,              // 直線部 4 個 × 2 本
